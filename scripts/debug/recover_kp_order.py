...
def curl(url: str) -> str:
    """Pobiera treść URL jako tekst."""
    result = subprocess.run(
        ["curl", "-s", url], capture_output=True, text=True, timeout=40
    )
    return result.stdout


def curlb(url: str, filename: str) -> None:
    """Pobiera URL do pliku."""
    subprocess.run(["curl", "-s", "-o", filename, url], timeout=40)
...
breeds = [
    ...
    "setter/english", "dalmatian", "doberman", "greatdane", "bullterrier",
    "pointer/german", "weimaraner", "newfoundland", "mastiff/bull",
]
    ...
    for k in range(4):
        try:
            ...
            for i, kp in enumerate(p2.keypoints):
                ...
                acc[i, 0] += wgt * (kp.x / width)
                acc[i, 1] += wgt * (kp.y / height)
                accv[i] += wgt
            n_used += 1
        except Exception:
            pass
