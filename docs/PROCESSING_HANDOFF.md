...
Полный перечень всех **1296** видео — в файле репозитория **`docs/video_manifest.csv`**
(колонки `emotion, video, fid, drive_link`). Это и есть «карта» где что лежит:

| эмоция | видео | из них с прямой ссылкой (`fid`) |
|--------|-------|--------------------------------|
| neutral  | 252 | 162 |
| sad      | 251 | 189 |
| happy    | 251 | 166 |
...
- **883 видео** имеют прямой Drive file ID (`fid`) → колонка `drive_link` даёт ссылку
  `https://drive.google.com/uc?id=<fid>`, качается напрямую (`gdown <fid>` или Drive API).
- **413 видео** (в основном старые метки: mafin/danek/masha/anton) — только по имени
  файла; их Drive находит **поиском по имени** внутри общего фолдера `DOGS`
  (см. путь A ниже — это делает `build_150plus.py` автоматически).
